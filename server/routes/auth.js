...
router.get("/kakao/callback", async (req, res) => {
    ...
    try {
      ...
      res.redirect(
        `https://172.10.7.34:3000/auth/kakao/callback?token=${accessToken}&userId=${user._id}&name=${user.name}`
      );
...
router.get("/naver/callback", async (req, res) => {
    ...
    try {
      ...
      res.redirect(
        `https://172.10.7.34:3000/auth/naver/callback?token=${accessToken}&userId=${user._id}&name=${user.name}`
      );
